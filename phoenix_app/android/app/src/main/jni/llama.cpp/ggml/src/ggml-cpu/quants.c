...
void dequantize_row_i2_s(const uint8_t * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t n, float i2_scale) {
    ...
    while (done < n) {
        ...
        const int64_t cols1 = blk_e >= 64  ? 32 : (blk_e > 32 ? blk_e - 32 : 0);
        const int64_t cols2 = blk_e >= 96  ? 32 : (blk_e > 64 ? blk_e - 64 : 0);
        const int64_t cols3 = blk_e >= 128 ? 32 : (blk_e > 96 ? blk_e - 96 : 0);
...
void quantize_row_i8_s(const float * GGML_RESTRICT x, void * GGML_RESTRICT y, int64_t n, float * act_scales, int32_t * act_sums) {
    int8_t * dst = (int8_t *)y;
    double max = 0.00001;
    for (int i = 0; i < n; ++i) {
...
void quantize_row_i8_s_4x1(const float * GGML_RESTRICT x, void * GGML_RESTRICT y, int64_t n, float * act_scales, int32_t * act_sums) {
    int8_t * dst = (int8_t *)y;
    double max = 0.00001;
    for (int i = 0; i < n; ++i) {
        double av = fabs((double)x[i]);
