...
static const struct ggml_type_traits type_traits[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_I2_S] = {
        .type_name                = "i2_s",
        .blck_size                = 4,       // 4 ternary weights packed per byte (2 bits each)
        .type_size                = sizeof(int8_t),
        .is_quantized             = true,
        .to_float                 = NULL, // dequantize_row_i2_s has non-standard signature (needs scale param)
    },
    [GGML_TYPE_I8_S] = {
        .type_name                = "i8_s",
        .blck_size                = 1,
...
size_t ggml_nbytes(const struct ggml_tensor * tensor) {
    ...
    // I2_S tensors store a per-tensor float scale after the packed data
...
enum ggml_type ggml_ftype_to_ggml_type(enum ggml_ftype ftype) {
    ...
    switch (ftype) {
        ...
        case GGML_FTYPE_MOSTLY_I2_S:          wtype = GGML_TYPE_I2_S;  break;
