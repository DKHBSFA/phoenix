...
static void ggml_compute_forward_mul_mat_one_chunk(
    ...
    // BitNet I2_S: extract activation scale/sum pointers and weight scale
    const bool is_i2s = (type == GGML_TYPE_I2_S);
    const int64_t total_src1_rows = ne11 * ne12 * ne13;
    const float   * i2s_act_scales = NULL;
    const int32_t * i2s_act_sums   = NULL;
    const float   * i2s_weight_scale = NULL;
    if (is_i2s) {
        i2s_act_scales    = (const float   *)((const char *)wdata + total_src1_rows * ne10);
        i2s_act_sums      = (const int32_t *)((const char *)i2s_act_scales + total_src1_rows * sizeof(float));
        // Weight scale is stored at offset ne00*ne01/4 in src0 data (one float per tensor)
        i2s_weight_scale  = (const float   *)((const char *)src0->data + (ne00 * ne01 / 4));
    }
    ...
    for (int64_t iir1 = ir1_start; iir1 < ir1_end; iir1 += blck_1) {
        for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
            for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir1_end; ir1 += num_rows_per_vec_dot) {
                ...
                if (is_i2s) {
                    // BitNet I2_S path: special row stride and activation scaling
                    const int64_t row_idx = i13 * ne12 * ne11 + i12 * ne11 + i11;
                    const char * src1_col_i2s = (const char *)wdata + row_idx * ne10;

                    // Batch 16 rows if possible
                    if (iir0 + blck_0 - 1 < ir0_end) {
                        vec_dot(ne00, &tmp[0], 1,
                                src0_row + iir0 * nb01, nb01,
                                src1_col_i2s, 0, 16);
                        for (int row = 0; row < 16; row++) {
                            tmp[row] = (tmp[row] - i2s_act_sums[row_idx]) / i2s_act_scales[row_idx] * (*i2s_weight_scale);
                        }
                    } else {
                        ...
                        for (int64_t ir0 = iir0; ir0 < iir0 + blck_0 && ir0 < ir0_end; ir0++) {
                            vec_dot(ne00, &tmp[ir0 - iir0], 0,
                                    src0_row + ir0 * nb01, 0,
                                    src1_col_i2s, 0, 1);
                            tmp[ir0 - iir0] = (tmp[ir0 - iir0] - i2s_act_sums[row_idx]) / i2s_act_scales[row_idx] * (*i2s_weight_scale);
                        }
                    }
                    memcpy(&dst_col[iir0], tmp, (MIN(iir0 + blck_0, ir0_end) - iir0) * sizeof(float));
                } else {
                    // Standard quantized path
...
UseGgmlGemm1:;
    ...
    if (src1->type != vec_dot_type) {
        ...
        if (src0->type == GGML_TYPE_I2_S) {
            // BitNet I2_S: quantize activations to I8_S with per-row scale and sum
            // wdata layout: [quantized i8 data | act_scales array | act_sums array]
            const int64_t total_rows = ne11 * ne12 * ne13;
            float   * act_scales = (float   *)(wdata + total_rows * ne10);
            int32_t * act_sums   = (int32_t *)((char *)act_scales + total_rows * sizeof(float));
                ...
                case GGML_OP_MUL_MAT:
                    {
                        ...
                        if (node->src[1]->type != vec_dot_type) {
                            ...
                            if (node->src[0]->type == GGML_TYPE_I2_S) {
                                const int64_t n_rows = ggml_nelements(node->src[1]) / node->src[1]->ne[0];
                                cur += n_rows * (sizeof(float) + sizeof(int32_t));
                            }
                        }
                    } break;
                case GGML_OP_MUL_MAT_ID:
